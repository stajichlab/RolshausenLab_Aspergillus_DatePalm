...
group <- read_csv("Atub_PopAssigned.csv",col_names=TRUE, col_types='ci') %>% mutate(Pop=sprintf("Pop_%d",Pop))
onelabsetrep <- as.data.frame(group)
#colnames(group) <- c("Strain","GroupAssign")
inds <- read.table("structure/Atub_v1.All.SNP.fam",header=F, stringsAsFactors=F)
...
p1 <- plotQ(flist[2],returnplot=T,exportplot=T,basesize=10,useindlab=T,showindlab=T,showlegend=T,sharedindlab=T,ordergrp=T,indlabsize=5,
            ...
            grplab=onelabsetrep,grplabsize=3,linesize=0.8,pointsize=8,outputfilename="Rmuc.combined.groupK2", imgtype="pdf",
...
p1 <- plotQ(flist[3],returnplot=T,exportplot=T,basesize=10,useindlab=T,showindlab=T,showlegend=T,sharedindlab=T,ordergrp=T,indlabsize=5,
            clustercol=clist$shiny,
            grplab=onelabsetrep,grplabsize=3,linesize=0.8,pointsize=8,outputfilename="Rmuc.combined.groupK3", imgtype="pdf",
            height=10,width=100,exportpath="structure")

p1 <- plotQ(flist[c(2:4)],imgoutput="join",returnplot=T,exportplot=T,basesize=9,ordergrp=T,useindlab=T,showindlab=T,showlegend=T,sharedindlab=T,
            clustercol=clist$shiny,
          grplab=onelabsetrep,grplabsize=4,linesize=0.8,pointsize=8,outputfilename="Rmuc.combined.groups_2-4", imgtype="pdf",exportpath="structure",
          height=10,width=100)

# make this a look/function at some point
p <- plotQMultiline(flist[2], returnplot=T,spl=25,useindlab=T,showlegend=T,
                    clustercol=clist$shiny,
                    imgtype="pdf",exportplot=T,sortind="Cluster1",grplab=onelabsetrep,grplabsize=8,ordergrp=T,
                    outputfilename="Rmuc.joined_multiline.K_2",exportpath="structure")
p <- plotQMultiline(flist[3], returnplot=T,spl=25,useindlab=T,showlegend=T,
                    clustercol=clist$shiny,
                    imgtype="pdf",exportplot=T,sortind="Cluster1",grplab=onelabsetrep,grplabsize=8,ordergrp=T,
                    outputfilename="Rmuc.joined_multiline.K_3",exportpath="structure")
