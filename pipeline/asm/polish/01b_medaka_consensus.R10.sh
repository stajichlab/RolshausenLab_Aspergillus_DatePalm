#!/usr/bin/bash -l
#SBATCH -N 1 -p gpu -n 1 -c 2 --mem 64gb --out logs/medaka_cons.%a.log  --gres=gpu:1 --time 2:00:00 -a 2

module load medaka/2.0.1-gpu
...
do
	...
	if [ ! -s $HDF ]; then
	    time medaka consensus $BAM $HDF --threads $CPU
	fi
	if [ ! -s $POLISHED ]; then
