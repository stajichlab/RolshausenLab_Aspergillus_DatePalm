...
do
  ...
  -f 6 -b12 -c1 --ultra-sensitive --long-reads
  fi
  ...
  if [ ! -f $OUTSEARCH/$STRAIN.Viral.diamond.tsv ]; then
	  diamond blastx --db $VIRALDB -q $UNMAPPEDASM/$STRAIN/scaffolds.fasta --out $OUTSEARCH/$STRAIN.Viral.diamond.tsv -f 6 -b12 -c1 --ultra-sensitive --long-reads
  fi
done
